...
def process_data(file):
    ...
    if missing:
        ...
        st.stop()
    df["Date of Publishing"] = pd.to_datetime(
        df["Date of Publishing"], dayfirst=True, errors="coerce"
    )
    ...
    final_df["Percent Change"] = (
        ...
        / final_df["Record Price"]
        * 100
    ).round(2)
    final_df["Distance from Target ($)"] = (
        (final_df["Current Price"] - final_df["target Price"])
        / final_df["target Price"]
        * 100
    ).round(2)
...
selected_companies = st.sidebar.multiselect(
    ...
    default=(st.session_state.watchlist + list(df["Company Name"].head(3).tolist()))[
        :3
    ],
)
...
period = st.sidebar.selectbox(
    "Time Period", ["All Time", "Last 3 Months", "Last 6 Months", "Last 1 Year"]
)
...
st.sidebar.download_button(
    "Download Report", csv, "Stock_Report.csv", "text/csv"
)
...
with tab1:
    ...
    with col2:
        st.metric("Avg Return", f"{df['Percent Change'].mean():+.2f}%")
    with col3:
    ...
    if df["Index"].nunique() > 1:
        ...
        fig_pie.update_layout(
            paper_bgcolor=bg_color, plot_bgcolor=bg_color, font_color=fg_color
        )
...
with tab2:
    ...
    for company in selected_companies:
        ...
        if row["Current Price"] >= row["target Price"]:
            ...
            alert = (
                f"QUALSCORE ALERT: {company} HIT TARGET! "
                f"Current: ₹{row['Current Price']:,} | Target: ₹{row['target Price']:,}"
            )
            ...
            st.error(
                f"NEAR TARGET! Only ₹{row['target Price'] - row['Current Price']:.0f} away!"
            )
        ...
        if not hist.empty:
            ...
            # 2️⃣ Target Price Line
            ax.axhline(
                row["target Price"],
                color="orange",
                linestyle="--",
                linewidth=2,
                label=f"Target ₹{row['target Price']}",
            )

            # 3️⃣ Dot on Publishing Date
            pub_date = pd.to_datetime(row["Date of Publishing"])
            if pub_date in hist.index:
                pub_price = hist.loc[pub_date, "Close"]
                dot_date = pub_date
            else:
                nearest_idx = hist.index.get_indexer([pub_date], method="nearest")[0]
            ...
            wa_msg = (
                f"*{company}* is at ₹{row['Current Price']:,} | "
                f"Target: ₹{row['target Price']:,}"
            )
            wa_url = f"https://wa.me/?text={wa_msg.replace(' ', '%20')}"
            st.markdown(
                f'<a href="{wa_url}" target="_blank"><button style="background:#25D366;color:white;padding:10px 20px;border:none;border-radius:10px;">Share on WhatsApp</button></a>',
                unsafe_allow_html=True,
            )
...
with tab3:
    ...
    st.bar_chart(
        filtered.set_index("Company Name")["Percent Change"].sort_values(ascending=False)
    )
    col1, col2 = st.columns(2)
    with col1:
        st.subheader("Top 5 Gainers")
        st.dataframe(
            filtered.nlargest(5, "Percent Change")[
    ...
    with col2:
        ...
        st.dataframe(
            filtered.nsmallest(5, "Percent Change")[
                ["Company Name", "Percent Change"]
            ],
            use_container_width=True,
        )
    ...
    norm = mcolors.TwoSlopeNorm(
        vmin=values.min(), vcenter=0, vmax=values.max()
    )
...
with tab4:
    ...
    try:
        ...
        if avg > 0.1:
            ...
            st.info(f"Overall: Neutral ({avg:+.2f})")
    except Exception:
...
with tab_portfolio:
    ...
    with col2:
        buy_price = st.number_input(
            "Your Buy Price", value=float(row["Record Price"])
        )
...
with tab_chat:
    ...
    if prompt := st.chat_input(
        "Ask anything: 'Best stock?', 'Target hit?', 'Nifty?', 'Reliance?'"
    ):
        ...
        matched = False
        for _, row in df.iterrows():
            if row["Company Name"].lower() in p or row[
                "Ticker"
            ].lower().replace(".ns", "").replace(".bo", "") in p:
                status = (
                    "TARGET HIT!"
                    if row["Current Price"] >= row["target Price"]
                    else "NEAR TARGET!"
                    if row["Current Price"]
                    >= row["target Price"] * 0.95
                    else "On Track"
                )
                reply = (
                    f"**{row['Company Name']}**\n"
        ...
        if not matched:
            if any(x in p for x in ["best", "top", "gainer"]):
                top = df.loc[df["Percent Change"].idxmax()]
                reply = (
                    f"TOP GAINER: **{top['Company Name']}** "
                    f"+{top['Percent Change']:+.2f}%"
                )
            elif any(x in p for x in ["worst", "loser"]):
                bot = df.loc[df["Percent Change"].idxmin()]
                reply = (
                    f"WORST: **{bot['Company Name']}** "
                    f"{bot['Percent Change']:+.2f}%"
                )
                ...
                reply = (
                    f"TARGET HIT: {', '.join(hits) if hits else 'None yet!'}"
                )
            elif "nifty" in p or "sensex" in p:
                reply = f"NIFTY 50: ₹{nifty:,.0f}\nSENSEX: ₹{sensex:,.0f}"
            elif "profit" in p or "portfolio" in p:
                reply = (
                    "Go to Portfolio tab → enter shares & buy price → see your profit!"
                )
